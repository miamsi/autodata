...
with st.sidebar:
    ...
    if not api_key:
        st.warning("⚠️ GROQ_API_KEY tidak terdeteksi. Fitur LLM dinonaktifkan.")
    ...
    if uploaded_file and st.session_state.raw_data is None:
        with st.spinner("Menjalankan Pemetaan Skema (Fuzzy Matching)..."):
            try:
                ...
                st.success("Rekayasa Data Berhasil!")
            except Exception as e:
                st.error(f"Kesalahan Ingest Data: {str(e)}")

    if st.session_state.cleaned_data is not None:
        st.info(f"🧬 **Skema Hasil Sinkronisasi**\nPagu Default: `{st.session_state.detected_schema['budget']}`\nK/L: `{st.session_state.detected_schema['nmdept']}`\nSatker: `{st.session_state.detected_schema['nmsatker']}`")
...
if st.session_state.cleaned_data is None:
    st.info("👋 **Sistem Siap.** Silakan unggah berkas excel/csv laporan keuangan Anda.")
else:
    ...
    c4.metric("Proyeksi Sisa Belanja Akhir", format_short_rp(forecast['under_absorption']), f"Estimasi: {forecast['forecast_absorption_rate']:.1f}%", delta_color="inverse")
    ...
    with t_dash:
        ...
        with col_right:
            st.markdown("**Komposisi Pagu Efektif per Jenis Belanja**")
    ...
    with t_chat:
        st.subheader("💬 Asisten Cerdas Intelijen Anggaran")
                ...
                if "chart_config" in message and message["chart_config"]:
                    ...
                    try:
                        if cfg["chart_type"] == "bar":
                            st.plotly_chart(px.bar(message["df_result"], x=cfg["x"], y=cfg["y"], color=cfg.get("color"), title=cfg["title"], orientation=cfg.get("orientation", "v")), use_container_width=True)
                        elif cfg["chart_type"] == "line":
                            st.plotly_chart(px.line(message["df_result"], x=cfg["x"], y=cfg["y"], color=cfg.get("color"), title=cfg["title"]), use_container_width=True)
                        elif cfg["chart_type"] == "pie":
                            st.plotly_chart(px.pie(message["df_result"], values=cfg["y"], names=cfg["x"], title=cfg["title"]), use_container_width=True)
                    except:
                        pass

        if user_prompt := st.chat_input("Tanyakan sesuatu (Contoh: 'Tampilkan 5 satker dengan pagu terbesar')"):
            with st.chat_message("user"):
                st.markdown(user_prompt)
            st.session_state.agent_chat_history.append({"role": "user", "content": user_prompt})
            
            with st.chat_message("assistant"):
                with st.spinner("Mengeksekusi SQL Agregat..."):
                    ...
                    if error_msg:
                        st.error(f"Kegagalan Kueri Data: {error_msg}")
                        ...
                        for col in disp_df.select_dtypes(include=['float', 'int']).columns:
                            if 'RATE' not in col and 'PERSEN' not in col and 'TAHUN' not in col and 'KODE' not in col:
                                disp_df[col] = disp_df[col].apply(format_rp)
                        ...
                        if chart_cfg:
                            try:
                                if chart_cfg["chart_type"] == "bar":
                                    st.plotly_chart(px.bar(res_df, x=chart_cfg["x"], y=chart_cfg["y"], color=chart_cfg.get("color"), title=chart_cfg["title"], orientation=chart_cfg.get("orientation", "v")), use_container_width=True)
                                elif chart_cfg["chart_type"] == "line":
                                    st.plotly_chart(px.line(res_df, x=chart_cfg["x"], y=chart_cfg["y"], color=chart_cfg.get("color"), title=chart_cfg["title"]), use_container_width=True)
                                elif chart_cfg["chart_type"] == "pie":
                                    st.plotly_chart(px.pie(res_df, values=chart_cfg["y"], names=chart_cfg["x"], title=chart_cfg["title"]), use_container_width=True)
                            except:
                                chart_cfg = None
                                
                        st.session_state.agent_chat_history.append({
                            "role": "assistant", "content": insight,
                            "df_result": disp_df, "chart_config": chart_cfg
                        })
    ...
    with t_forecast:
        ...
        if not forecasts.empty:
            ...
            fig_f.add_trace(go.Scatter(x=full_track['month'], y=full_track['cumulative'], name='Proyeksi', line=dict(color='#f59e0b', dash='dash')))
    ...
    with t_sim:
        ...
        with sc_col1:
            ...
            target_abs = st.slider("Target Batas Penyerapan (%)", 70, 100, 95)
        ...
        with sc_col2:
            ...
            st.metric("Simulasi Anggaran Terserap", format_rp(sim_res['sim_realization']), f"Total Serap: {sim_res['sim_absorption_rate']:.1f}%")
    ...
    with t_rev:
        ...
        st.metric("Target Sisa Bulanan", format_rp(rev_res['required_monthly_spending']), f"Dihitung untuk sisa {rev_res['remaining_months']} bulan aktif")
    ...
    with t_bottleneck:
        try:
            ...
            # Fallback jika library matriks belum terinisialisasi
            st.dataframe(bottlenecks, use_container_width=True)

    with t_report:
        if st.button("🚀 Buat Laporan Dokumen"):
            with st.spinner("AI Llama 3 sedang membuat naskah laporan..."):
                entity_col = schema['kdsatker'] if schema['kdsatker'] else (schema['org'][0] if schema['org'] else 'INDEX')
        ...
        if st.session_state.executive_report:
            st.download_button(
                ...
                file_name="Laporan_Eksekutif.docx", 
