"""
Budget Intelligence Agent - Conversational SQL Engine & AI Chart Agent (OSS 20B Optimized)
"""
...
class BudgetQueryEngine:
    def __init__(self, df: pd.DataFrame, schema: Dict[str, Any], api_key: str):
        ...
        self.model_name = "gpt-oss-20b" # HARDCODED SEBAGAIMANA PERMINTAAN ANDA

    def _extract_sql_from_text(self, text: str) -> str:
        """Fungsi ekstraksi kokoh jika OSS Model gagal membuat blok markdown"""
        match = re.search(r'
...
if st.session_state.cleaned_data is not None:
    ...
    with t_chat:
        ...
        if user_prompt := st.chat_input("Tanyakan sesuatu (Contoh: 'Tampilkan 10 satker dengan pagu terbesar')"):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Mengeksekusi SQL Agregat..."):
                    ...
                    if error_msg:
                        st.error(f"Kegagalan Kueri Data. LLM 20B gagal mematuhi format DuckDB.\n{error_msg}")
    ...
    st.info("💡 Hubungkan berkas laporan anggaran Anda di panel menu bagian kiri untuk memulai eksekusi data.")
