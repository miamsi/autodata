...
# Kamus Sinonim Metadata Komprehensif (Sesuai Konteks Kemenkeu/DIPA)
COLUMN_METADATA = {
...
def detect_budget_schema(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    def find_match(target_key: str) -> str:
        if target_key not in COLUMN_METADATA:
            return None
    ...
    # Masukkan sisa hierarki organisasi (tanpa crash karena metadata sudah lengkap)
    for k in ["NMDEPT", "NMUNIT", "NMKANWIL", "NMKPPN", "NMSATKER", "KDDEPT", "KDUNIT", "KDKANWIL", "KDKPPN", "KDSATKER"]:
        m = find_match(k)
        if m and m not in schema['org']: 
            schema['org'].append(m)
            
    # Masukkan sisa hierarki program/akun
    for k in ["NMPROGRAM", "NMGIAT", "NMOUTPUT", "NMAKUN", "NMSDANA2", "KDPROGRAM", "KDGIAT", "KDOUTPUT", "KDAKUN", "KDSDANA"]:
        m = find_match(k)
        if m and m not in schema['account']: 
            schema['account'].append(m)
...
def clean_and_prepare_data(df: pd.DataFrame, schema: Dict[str, Any]) -> pd.DataFrame:
    ...
    # Pastikan data keuangan adalah numerik
    if schema['budget']:
